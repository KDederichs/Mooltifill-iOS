...
//
//  Created by Kai Dederichs on 10.04.22.
//
...
class CredentialProviderViewController: ASCredentialProviderViewController {
    ...
    */
    override func prepareCredentialList(for serviceIdentifiers: [ASCredentialServiceIdentifier]) {
        ...
        manager.service.getCredentials(service: url!.host!, login: nil, callback: {
            let passwordCredential = ASPasswordCredential(user: $0, password: $1)
            self.extensionContext.completeRequest(withSelectedCredential: passwordCredential, completionHandler: nil)
        })

        debugPrint(serviceIdentifiers)
        //check locked. If lock ask for unlock and then fetch credentials
    }
